...
// delete column using id.
{
	...
	$stmt = 'DELETE FROM notification WHERE id = :id';
...
// prepared stmt is execute with criteria and display deleted statement if the criteria get executed using strong
	if($preparedstmt->execute($criteria))  {
        ?>
                   <strong>Deleted successfully.Pree Ok!! <button> <a href="view_notification.php">OK</a></button></strong>
        <?php
        }
        else
        {
...
?>
  <?php 
//notification_table_generator is declared
...
// database name group_project is connected using pdo connnection
 ...
 $tg->setHeadings(['id','Message','fName','lName','Date']);
  // statement of table column is selected from notoficatio
